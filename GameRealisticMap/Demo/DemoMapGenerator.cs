...
namespace GameRealisticMap.Demo
{
    ...
    public class DemoMapGenerator
    {
        ...
        public void CreateInto(BuildContext context, string name, Dictionary<BuildingTypeId,List<Vector2>> buildingTypicalSizes)
        {
            ...
            CreateSampleSingle(context, places, polygon => new IceSurfaceData(polygon), new Vector2(8 * 512, 0));
            CreateSampleSingle(context, places, polygon => new ScreeData(polygon), new Vector2(9 * 512, 0));
            ...
            CreateBuildings(context, places, roads, context.Area, buildingTypicalSizes, areas);

            context.SetData(new RoadsData(roads));
            context.SetData(new CitiesData(places));
            context.SetData(new CategoryAreaData(areas));
#if DEBUG
            PreviewRender.RenderHtml(context, "demo.html").Wait();
#endif
        }

        private void CreateBuildings(BuildContext context, List<City> places, List<Road> roads, ITerrainArea area, Dictionary<BuildingTypeId, List<Vector2>> buildingTypicalSizes, List<CategoryArea> areas)
        {
            ...
            foreach (var pair in buildingTypicalSizes)
            {
                var id = pair.Key;
                var hasLabel = false;
                var start = new TerrainPoint(x, centerY - 206);
                foreach (var size in pair.Value)
                {
                    var patternSize = size.X * 2 + tertiary.ClearWidth + 64;
                    if (x + patternSize > area.SizeInMeters)
                    {
                        FlushArea(areas, centerY, x, pair.Key, start);

                        x = 0;
                        centerY += 412f;

                        start = new TerrainPoint(x, centerY - 206);
        ...
        private static void FlushArea(List<CategoryArea> areas, float centerY, float x, BuildingTypeId type, TerrainPoint start)
        {
            switch (type)
            {
                // Not generated, use Residential as it's a more likely combinaison
                case BuildingTypeId.WaterTower:
                case BuildingTypeId.BusStopShelter:
                case BuildingTypeId.IndividualGarage:
                case BuildingTypeId.Church:
                case BuildingTypeId.Shed:
                case BuildingTypeId.Hut:
                case BuildingTypeId.HistoricalFort:
                case BuildingTypeId.RadioTower:
                    type = BuildingTypeId.Residential;
                    break;

                // Generated by OSM part
                case BuildingTypeId.Retail:
                case BuildingTypeId.Residential:
                case BuildingTypeId.Military:
                case BuildingTypeId.Commercial:
                case BuildingTypeId.Industrial:
                case BuildingTypeId.Agricultural:
                    break;

            }

            var end = new TerrainPoint(x, centerY + 206);
            if (start.X != end.X)
            {
                areas.Add(new CategoryArea(type, new List<TerrainPolygon> { TerrainPolygon.FromRectangle(start, end) }));
            }
        }
