...
namespace GameRealisticMap.Studio.Shared
{
    public abstract class PersistedDocumentOverridable : Document, IPersistedDocument
    {
        ...
        public override async Task<bool> CanCloseAsync(CancellationToken cancellationToken)
        {
            if (IsDirty)
            {
                var response = MessageBox.Show(string.Format(Gemini.Properties.Resources.DocumentDirtyCloseConfirmText, FileName), Gemini.Properties.Resources.DocumentDirtyCloseConfirmTitle, MessageBoxButton.YesNoCancel);
        ...
        public async virtual Task Save(string filePath)
        {
            FilePath = filePath;
            FileName = Path.GetFileName(filePath);
            UpdateDisplayName();
        ...
        public void AssumeSavedAs(string filePath)
        {
            FilePath = filePath;
            FileName = Path.GetFileName(filePath);
            UpdateDisplayName();
